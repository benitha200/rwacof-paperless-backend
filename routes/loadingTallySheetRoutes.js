...
router.post('/', validateLoadingTallySheet, async (req, res, next) => {
  ...
  try {
    // Try to create new Loading List
    ...
    if (error.code === 'P2002') {
      try {
        ...
        next(new AppError(`Failed to update Loading List: ${updateError.message}`, 400));
      }
    } else if (error.code === 'P2003') {
      next(new AppError('A foreign key constraint failed. Please check the shipmentId.', 400));
    } else {
      next(new AppError(`Failed to create Loading List: ${error.message}`, 500));
    }
  }
});
...
router.get('/', async (req, res, next) => {
  try {
    ...
    next(new AppError('Failed to fetch Loading Lists', 500));
  }
});
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    if (!loadingTallySheet) {
      throw new AppError('Loading List not found', 404);
    }
    res.json(loadingTallySheet);
  } catch (error) {
    next(error);
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    next(new AppError('Failed to update Loading List', 500));
  }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    next(new AppError('Failed to delete Loading List', 500));
  }
});
